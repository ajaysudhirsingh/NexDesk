...
FROM base AS build
# Install ALL dependencies (including devDependencies for build tools like craco)
RUN npm ci
COPY . .
# Build frontend
...
# Copy API package files and install production dependencies only
COPY --chown=nexdesk:nodejs api/package*.json ./api/
